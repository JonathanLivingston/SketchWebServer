...
public class RequestParser {
	...
	public static HttpRequest parseStream(InputStream stream)
		...
		try (Scanner scanner = new Scanner(stream);) {
			...
			if (!headerMatcher.matches()) {
				...
				request.setProtocol(RequestProtocol.valueOf(headerMatcher
						.group(2)));
			}
			while (scanner.hasNextLine()) {
				...
				if (valueMatcher.matches()) {
					request.putValue(valueMatcher.group(1),
